...
Mỗi dự án cần một môi trường ảo riêng. Nếu chưa có, hãy tạo và cài đặt thư viện:
```powershell
python -m venv venv
.\venv\Scripts\Activate.ps1
pip install openpyxl python-dotenv Pillow
```
...
- `.agent\skills\report-customer-pricing\script\Bao-Gia.py`: Script gốc (không đổi).
...
### Bước 1: Khởi tạo (Sync) & Cấu hình
1. Copy file script mẫu ra thư mục gốc:
```powershell
cp -Force .agent\skills\report-customer-pricing\script\Bao-Gia.py Bao-Gia.py
```
2. Thiết lập thông số công ty trong file `.env` (tạo mới bằng cách copy nội dung từ file `.env.example` nếu chưa có).

### Bước 2: Cập nhật dữ liệu (Update)
Cập nhật thông tin đơn hàng cụ thể vào file `Bao-Gia.py` tại thư mục gốc:
